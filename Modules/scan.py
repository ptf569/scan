...
def testssl(testssl, location, target, rescan):
    match = re.compile("^([^:]*)*")
    start = datetime.now()
    message = colored("[+] {0} : PERFORMING TESTSSL SCAN AT {1}\n".format(target, start), 'green')
    op = target.replace(':', '-')
    ...
    if os.path.isfile(data):
        if rescan == False:
            ...
            message = colored("[+] {0} : PERFORMING TESTSSL RESCAN ON HOST AT {1}\n".format(target, start), 'green')
...
def sslscan(location, target, rescan):
    match = re.compile("^([^:]*)*")
    start = datetime.now()
    message = colored("[+] {0} : PERFORMING SSLSCAN AT {1}\n".format(target, start), 'green')
    ...
    if os.path.isfile(data):
        if rescan == False:
            ...
            message = colored("[+] {0} : PERFORMING SSLSCAN RESCAN ON HOST AT {1}\n".format(target, start), 'green')
            app = '>>'
